...
class GradeImport implements ToCollection
{
    ...
    public function collection(Collection $collection)
    {
        ...
        //         echo print_r($this->header_check($value)) . '<br>';
        //     }
        // }
    ...
    public function grade_upload($collection)
    {
        ...
        $_headers = $collection[0];
        $_file_name = 'log/_log_' . str_replace(' ', '_', $_section->section->section_name)  . "_" . date('d_m_y') . '.log';
                        ...
                        foreach ($_headers as $_key => $value) {
                            ...
                            if ($_key > 5) { // Start on 6 index
                                ...
                                if ($_header_data['type'] != "Q0" && $_header_data['type'] != "none0") {
                                    if ($_check_details) {
                                        // Update Score
                                        $_save = GradeEncode::where($_score_details)->update(['score' => intval($_data[$_key])]);
                                        $_data_to_log[] = 'Updated Grades';
                                    } else {
                                        // Save Score
                                        $_score_details['score'] = intval($_data[$_key]);
                                        $_score_details['is_removed'] = 0;
                                        $_save = GradeEncode::create($_score_details);
                                        $_data_to_log[] = 'Saved Grades';
                                    }
                                }
                                $_data_to_log[] .= PHP_EOL; // Next line in log file
                            }
    ...
    public function header_check($_value)
    {
        ...
        switch ($_data[0]) {
            ...
            case 'Assignment':
                switch (trim($_data[2])) {
                    case str_contains($_data[2], 'Oral'):
                        $_label = 'O' . $_number;
                        break;
                    case str_contains($_data[2], 'Laboratory'):
                        $_label = 'L' . $_number;
                        break;
                    default:
                        $_label = 'R' . $_number;
