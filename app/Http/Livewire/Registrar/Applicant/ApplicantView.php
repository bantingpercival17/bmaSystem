...
class ApplicantView extends Component
{
    ...
    function dataFilter($search, $course, $category, $academic)
    {
        ...
        $databaseSecond = env('DB_DATABASE_SECOND');

        $applicantAccountTable = "$database.applicant_accounts";
        $tblDocuments = "$database.documents";
        $tblApplicantDetails = "$databaseSecond.applicant_detials";
        $tblApplicantDocuments = "$databaseSecond.applicant_documents";
        $tblApplicantNotQualifieds = "$databaseSecond.applicant_not_qualifieds";
        $tblApplicantPayment = "$databaseSecond.applicant_payments";
        $tblApplicantAlumia = "$databaseSecond.applicant_alumnias";
        $tblApplicantExamination = "$databaseSecond.applicant_entrance_examinations";
        $tblApplicantMedicalScheduled = "$databaseSecond.applicant_medical_appointments";
        $tblApplicantMedicalResult = "$databaseSecond.applicant_medical_results";
        ...
        switch ($category) {
            ...
            case 'approved':
                $selectFields = [
                    ...
                    WHERE $tblApplicantDocuments.applicant_id = applicant_accounts.id
                      ...
                      AND $tblApplicantDocuments.is_approved = 1
                    ...
                    WHERE $tblDocuments.department_id = 2
                      ...
                      AND $tblDocuments.year_level = (
                          SELECT IF($applicantAccountTable.course_id = 3, 11, 4)
                          FROM $applicantAccountTable
                          WHERE $applicantAccountTable.id = $tblApplicantDocuments.applicant_id
                      )
                ) AS documentCount"),
                ];
            ...
            case 'examination_payment':
                $dataList = $dataList->join($tblApplicantPayment, "$tblApplicantPayment.applicant_id", 'applicant_accounts.id')
                    ->where(function ($query) use ($databaseSecond) {
                        $query->whereNull("$databaseSecond.applicant_payments.is_approved")
                            ->orWhere("$databaseSecond.applicant_payments.is_approved", false);
                    })
                    ->where("$tblApplicantPayment.is_removed", false)
            ...
            case 'entrance_examination':
                $dataList = $dataList->join($tblApplicantExamination, "$tblApplicantExamination.applicant_id", 'applicant_accounts.id')
            ...
            case 'for_medical_schedule':
                $dataList = $this->examination_result($dataList, '>=')
                    ->union($this->senior_high_alumia($dataList))
            ...
            case 'waiting_for_medical_results':
                $dataList = $dataList->join($tblApplicantMedicalScheduled, "$tblApplicantMedicalScheduled.applicant_id", 'applicant_accounts.id')
            ...
            case 'non_pbm':
            case 'pbm':
                $fitValue = $category === 'unfit' ? 2 : 1;
                $dataList = $this->medical_result($dataList, $fitValue);
                if ($category === 'non_pbm') {
                    $dataList->where('applicant_accounts.strand', '!=', 'Pre-Baccalaureate Maritime Strand');
                } elseif ($category === 'pbm') {
                    $dataList->where('applicant_accounts.strand', 'Pre-Baccalaureate Maritime Strand');
                }
                break;
            ...
            case 'pending_result':
                $dataList = $dataList->join($tblApplicantMedicalResult, "$tblApplicantMedicalResult.applicant_id", 'applicant_accounts.id')
    ...
    {
        $tblApplicantExamination = env('DB_DATABASE_SECOND') . '.applicant_entrance_examinations';
        $tblApplicantExaminationAnswers = env('DB_DATABASE_SECOND') . '.applicant_examination_answers';
        $tblExaminationQuestionChoices = env('DB_DATABASE') . '.examination_question_choices';

        return $data->join($tblApplicantExamination, "$tblApplicantExamination.applicant_id", 'applicant_accounts.id')
            ->where("$tblApplicantExamination.is_removed", false)
            ->where("$tblApplicantExamination.is_finish", true)
            ->where(function ($query) use ($tblApplicantExaminationAnswers, $tblExaminationQuestionChoices, $tblApplicantExamination) {
                $query->select(DB::raw('COUNT(*)'))
                    ->from($tblApplicantExaminationAnswers)
                    ->join($tblExaminationQuestionChoices, "$tblExaminationQuestionChoices.id", '=', "$tblApplicantExaminationAnswers.choices_id")
                    ->where("$tblExaminationQuestionChoices.is_answer", true)
                    ->whereColumn("$tblApplicantExaminationAnswers.examination_id", "$tblApplicantExamination.id");
            }, $operation, function ($query) {
