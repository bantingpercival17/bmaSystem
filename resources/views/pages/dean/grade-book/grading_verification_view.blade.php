...
<li class="breadcrumb-item active" aria-current="page">
    <a href="{{ route('dean.grade-verification-view') }}?_academic={{ base64_encode(Auth::user()->staff->current_academic()->id) }}">
            ...
            <div class="table-responsive">
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <span class="text-primary fw-bolder">{{ $item->curriculum_subject->subject->subject_name }}</span>
                                <br>
                            ...
                            <td class="d-flex justify-content-between">
                                <div class="">
                                    ...
                                    <button type="button" class="btn btn-primary btn-sm btn-form-grade w-100 mt-2" data-bs-toggle="modal" data-bs-target=".grade-view-modal" data-grade-url="{{ route('dean.grading-sheet-view') }}?_subject={{ base64_encode($item->id) }}&_period=midterm&_preview=pdf&_form=ad1">
                                        MIDTERM</button>
                                    @else
                                    ...
                                    <button type="button" class="btn btn-primary btn-sm btn-form-grade w-100 mt-2" data-bs-toggle="modal" data-bs-target=".grade-view-modal" data-grade-url="{{ route('dean.grading-sheet-view') }}?_subject={{ base64_encode($item->id) }}&_period=finals&_preview=pdf&_form=ad2">
                                        FORM AD-02</button>
                                    @endif
                                </div>
                                <div class="ms-2">
                                    ...
                                    @if (count($item->grade_publish()) <= 0) <a href="{{ route('dean.grade-publish') }}?subject_class='{{ base64_encode($item->id) }}'&_subject={{ base64_encode($item->id) }}&_status=1&_period=finals" class="btn btn-info btn-sm text-white w-100  mt-2">PUBLISH</a>
                                        @endif
                                        <a href="{{ route('dean.grade-publish') }}?subject_class='{{ base64_encode($item->id) }}'&_subject={{ base64_encode($item->id) }}&_status=1&_period=finals" class="btn btn-info btn-sm text-white w-100  mt-2">PUBLISH</a>
                                        @else
                                        <form action="{{ route('dean.grade-verification') }}" method="get">
                                            ...
                                            <div class="row">
                                                <div class="col-md">
                                                    <a href="{{ route('dean.grade-verification') }}?subject_class='{{ base64_encode($item->id) }}'&_subject={{ base64_encode($item->id) }}&_status=1&_period=finals" class="btn btn-info btn-sm text-white w-100  mt-2">APPROVED</a>
                                                </div>
                                                <div class="col-md">
                                                    <input type="hidden" name="_status" value="0">
                                                    <button type="submit" class="btn btn-danger btn-sm w-100 mt-2">DISAPPROVED</button>
                                                </div>
                                            </div>
                                        </form>
                                        @endif
