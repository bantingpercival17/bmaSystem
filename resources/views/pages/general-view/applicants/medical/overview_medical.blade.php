...
<section>
                    ...
                    <thead>
                        <tr class="text-center">
                            ...
                            <th>
                                {{ strtoupper($item[0]) }}
                                <br>
                                <a href="{{ route('medical.export-medical-applicant-list') . '?category=' . $item[1] }}{{ request()->input('_academic') ? '&_academic=' . request()->input('_academic') : '' }}" class="badge bg-info">EXPORT</a>
                            </th>
                            @endforeach
                        </tr>
                    </thead>
                ...
                <div class="row">
                    <div class="col-6">
                        ...
                        <div class="form-group search-input">
                            <input type="search" class="form-control" placeholder="Search Pattern: Lastname, Firstname" name="_students">
                        </div>
                    </div>
                    <div class="col-4">
            ...
            <div class="d-flex justify-content-between mb-3">
                <div>
                    ...
                    <div class="d-flex justify-content-between">
                        @if (request()->input('_sort'))
                        <div>
                            <small>Sort By: </small> <span class="fw-bolder text-info">{{ ucwords(str_replace('-', ' ', request()->input('_sort'))) }}</span>
                        </div>
                        @endif
                        @if (request()->input('_year_level'))
                        <div class="ms-5">
                            <small>Year Level Result: </small> <span class="fw-bolder text-info">{{ request()->input('_year_level') }}</span>
                        </div>
                        @endif
                    </div>
                </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <form action="{{route('medical.applicant-schedule')}}" method="post">
                                    ...
                                    <div class="form-group">
                                        <small for="" class="fw-bolder text-muted">SCHEDULE DATE</small>
                                        <select name="date" id="" class="form-select form-select-sm">
                                            <option disabled selected>Select Date</option>
                                            @foreach($dates as $date)
                                            <option value="{{$date->date}}" {{ Auth::user()->staff->medical_appointment_slot($date->date) == $date->capacity ? 'disabled':'' }}>{{date($_format, strtotime($date->date))}} -   <span class="text-info fw-bolder">{{ Auth::user()->staff->medical_appointment_slot($date->date) }}</span><small class="text-secondary">/{{$date->capacity}}</small></option>
                                            @endforeach
                                        </select>
                            ...
                            <a href="{{ route('medical.applicant-appointment') }}?appointment={{ base64_encode($_data->medical_appointment->id) }}" class="btn btn-sm btn-outline-info mt-2">APPROVED</a>
                            @endif
                            ...
                            <a href="{{ route('medical.applicant-medical-result') . '?result=' . base64_encode(1) . '&applicant=' . base64_encode($_data->id) }}" class="btn btn-primary btn-sm w-100 mb-2">FIT</a>
                            <a class="btn btn-danger btn-sm w-100 mb-2 btn-medical" data-applicant="{{ base64_encode($_data->id) }}" data-bs-toggle="modal" data-bs-target=".modal-medical-fail">FAIL</a>
                            @endif
                            <span class="badge bg-secondary">{{ $_data->medical_result->created_at->format('F d,Y') }}</span>
                            @else
                            <a href="{{ route('medical.applicant-medical-result') . '?result=' . base64_encode(1) . '&applicant=' . base64_encode($_data->id) }}" class="btn btn-primary btn-sm w-100 mb-2">FIT</a>
                            <a class="btn btn-danger btn-sm w-100 mb-2 btn-medical" data-applicant="{{ base64_encode($_data->id) }}" data-bs-toggle="modal" data-bs-target=".modal-medical-fail">FAIL</a>
                            <a class="btn btn-info btn-sm w-100 text-white mb-2 btn-medical" data-applicant="{{ base64_encode($_data->id) }}" data-bs-toggle="modal" data-bs-target=".modal-medical-pending">PENDING</a>
                            @endif
    ...
    @foreach ($_courses as $_course)
    <div class="col-md">
        <a href="{{ route('applicant-virtual-briefing') }}?_course={{ base64_encode($_course->id) }}{{ request()->input('_academic') ? '&_academic=' . request()->input('_academic') : '' }}">
            <div class="card  iq-purchase" data-iq-gsap="onStart" data-iq-position-y="50" data-iq-rotate="0" data-iq-trigger="scroll" data-iq-ease="power.out" data-iq-opacity="0">
                <div class="card-body">
