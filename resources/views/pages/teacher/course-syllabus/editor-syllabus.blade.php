...
<div class="card-body">
    @php
    $_array_tab = [['A', 'part-one', 'COURSE SPECIFICATION'], ['B', 'part-two', 'COURSE OUTLINE AND TIMETABLE'],['C', 'part-three', 'COURSE SYLLABUS'],['E','part-five','Materials']];
    @endphp
    ...
    <ul class="nav nav-tabs nav-fill" id="myTab-three" role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link {{ $key == 0 ? 'active' : '' }}" id="{{ $tab[1] }}" data-bs-toggle="tab" href="#{{ $tab[1] }}-content" role="tab" aria-controls="home" aria-selected="{{ $key == 0 ? true : false }}">PART
                {{ $tab[0] }}</a>
        </li>
        @endforeach
    ...
    <div class="tab-content" id="myTabContent-4">
        ...
        <div class="tab-pane fade show {{ $key == 0 ? 'active' : '' }}" id="{{ $item[1] }}-content" role="tabpanel" aria-labelledby="{{ $item[1] }}">
            <label for="" class="h5 fw-bolder text-primary">{{ $item[2] }}</label>
            <div class="content-tool float-end">
                <a href="{{ route('teacher.course-syllabus-report') . '?_course_syllabus=' . base64_encode($_course_syllabus->id) }}&_part={{ base64_encode($item[1]) }}" class="btn btn-primary btn-sm">GENERATE PART {{ $item[0] }}</a>
            </div>
            <div class="content">
                @yield($item[1])
            </div>
