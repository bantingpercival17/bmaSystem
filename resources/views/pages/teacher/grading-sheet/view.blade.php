                ...
                <div class="row">
                                ...
                                <div class="row ">
                                    <div class="col-md-8">
                                        <div class="form-group ">
                                            ...
                                            <input class="form-control form-control-sm border border-primary" type="file" id="customFile" name="_file_grade" required {{ $_grade_status }}>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        <div class="form-group p-0 m-0">
                            ...
                            <label for="" class=""><a class="badge bg-primary" href="{{ route('teacher.export-grade') . '?_subject=' . request()->input('_subject') . '&_period=' . request()->input('_period') }}">Grading
            ...
            <div class="col-md-6">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <a href="{{ route('teacher.grading-sheet') }}?_subject={{ base64_encode($_subject->id) }}&_period=midterm" class="badge {{ request()->input('_period') == 'midterm' ? 'bg-info text-white' : 'bg-secondary' }} me-2 mt-2">MIDTERM</a>
                        <a href="{{ route('teacher.grading-sheet') }}?_subject={{ base64_encode($_subject->id) }}&_period=finals" class="badge {{ request()->input('_period') == 'finals' ? 'bg-info text-white' : 'bg-secondary' }} me-2 mt-2">FINALS</a>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="row">
                        <div class="col-md">
                            <p>
                                ...
                                <br>
                                <small>DATE SUBMISSION :</small> <span class="text-muted"><b>{{ $_subject_data->created_at->format('M d, Y') }}</b></span>
                            </p>
                        </div>
                        <div class="col-md">
                            ...
                            <br>
                            <small>VERIFIED DATE :</small> <span class="text-muted"><b>{{ $_subject_data->updated_at->format('M d, Y') }}</b></span>
                        </div>
                    </div>
                ...
                <table style="height: 700px;">
                    <thead>
                        ...
                        <tr>
                            ...
                            @foreach ($_columns as $col)
                            @for ($i = 1; $i <= $col[2]; $i++) @php $cNum +=1; @endphp <th class=" text-center table-bordered text-primary fw-bolder">
                                {{ strtoupper($col[1]) . $i }}
                                </th>
                                @endfor
                                @endforeach
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th class={{ $_student->student->enrollment_assessment_paid->enrollment_cancellation ? 'text-danger fw-bolder' : 'text-primary fw-bolder' }}>
                            ...
                            @else
                            @foreach ($_columns as $col)
                            @for ($i = 1; $i <= $col[2]; $i++) <td class="text-center table-bordered">
                                @php
                                $_score = $_student->student->subject_score([$_subject->id, request()->input('_period'), $col[1] . $i]);
                                @endphp
                                <input type="text" class="score-cell" style="width: 38px; font-size:12px" value="{{ $_score }}" data-student="{{ $_student->student->id }}" data-category="{{ $col[1] . $i }}" data-section="{{ $_subject->id }}" {{ $_grade_status }}>
                                </td>
                                @endfor
                                @endforeach
                                @endif
