                        ...
                        <tbody>
                            @foreach ($_courses as $_course)
                                <tr>
                                    ...
                                    @foreach ($_table_content as $key => $item)
                                        @php
                                            $_route_link = route('medical.student-medical-appointment') . '?' . (request()->input('_academic') ? '_academic=' . request()->input('_academic') . '&' : '') . 'view=' . $item[0];
                                        @endphp
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md">
                                        ...
                                        @else
                                            @php
                                                $_dates = ['2023-05-15', '2023-05-17', '2023-05-19'];
                                                $_format = 'D F d, Y';
                                            @endphp
                                            @foreach ($_dates as $_date)
                                                <div class="col-md">
                                                    <h4>
                                                        <span
                                                            class="text-info fw-bolder">{{ Auth::user()->medical_appointment_slot($_date) }}</span><small
                                                            class="text-secondary">/20</small>
                                                    </h4>

                                                    @if (Auth::user()->medical_appointment_slot($_date) >= 20)
                                                        <span class="badge bg-secondary text-white">
                                                            {{ date($_format, strtotime($_date)) }} This schedule is
                                                            full</span>
                                                    @else
                                                        <a href="{{ route('applicant.medical-schedule') }}?_date={{ $_date }}"
                                                            class="btn btn-sm btn-primary">
                                                            {{ date($_format, strtotime($_date)) }}</a>
                                                    @endif
                                                </div>
                                            @endforeach
                                        @endif
